 ...
 module.exports.takeQuery = function takeQuery (path, params={}){
	switch(path){
		...
		case '/organization_programs':
			return `SELECT * FROM Organizations JOIN Programs ON Programs.Organization_ID=Organizations.Organization_ID WHERE Organizations.Organization_NAME;`;
			break;

		// Q4: What other programs does this contact info relate to? 
		case '/contact_conections':
			return `SELECT * FROM Programs_Contacts JOIN Programs ON Programs.Program_ID=Programs_Contacts.program_ID JOIN Contacts ON Programs_Contacts.Contact_ID=Contacts.Contact_ID JOIN Organizations ON Organizations.Organization_ID=Contacts.Organization_ID WHERE Contacts.Email;`;
			break;
